...
export default function StatusUpdateModal({ isOpen, onClose, order, onConfirm }: StatusUpdateModalProps) {
    ...
    const currentOrderStatus = order?.status.toLowerCase() || 'pending';

    // Sync state whenever modal opens or order changes
    React.useEffect(() => {
        if (isOpen && order) {
            const status = order.status.toLowerCase();
            setTargetStatus(status);
            setStatusMessage(DEFAULT_MESSAGES[status] || '');
    ...
    const headerRight = (
        <div className="flex items-center gap-2">
            ...
            <button
                ...
                className="flex items-center justify-center w-8 h-8 text-[#3f9733] hover:text-[#308026] bg-[#3f9733]/5 border border-[#3f9733]/10 hover:bg-[#3f9733]/10 rounded-lg transition-all active:scale-95 disabled:opacity-50"
                title={isResending ? 'Resending...' : 'Resend Email Notification'}
            >
                {isResending ? (
                    <div className="w-3 h-3 border-2 border-[#3f9733]/30 border-t-[#3f9733] rounded-full animate-spin" />
                ) : (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 border border-dotted border-gray-300 divide-x divide-y divide-dotted divide-gray-300 rounded-[6px] overflow-hidden">
                        {statuses.map((s) => (
                            <button
                                ...
                                className={`flex flex-col gap-1 p-4 transition-all text-left group relative ${
                                    targetStatus === s 
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    {currentOrderStatus === s && (
                                        <div className={`w-1.5 h-1.5 rounded-full ${targetStatus === s ? 'bg-white' : 'bg-green-500'}`} title="Current Status" />
                                    )}
                                </div>
