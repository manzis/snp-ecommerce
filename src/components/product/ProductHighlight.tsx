        ...
        >
          <div key={activeTab} className="relative h-full w-full pointer-events-none animate-in fade-in duration-500">
            {highlights[activeTab]?.type === 'video' ? (
              ...
              <Image
                src={(highlights[activeTab]?.src || '/images/protein.webp').trim()}
              ...
              <div className="absolute bottom-[14px] left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 px-3 py-2 rounded-full bg-black/20 backdrop-blur-xl border border-white/5 pointer-events-auto">
                {highlights.map((_, idx) => (
                  <button
                    ...
                    className={`relative h-1.5 rounded-full outline-none transition-all duration-500 overflow-hidden ${
                      activeTab === idx ? 'w-8 bg-white opacity-100' : 'w-2 bg-white/40 opacity-40'
                    }`}
                  >
                  </button>
                ))}
        ...
        <div className="hidden lg:flex flex-col gap-[6px] lg:flex-[1] h-full">
          {highlights.slice(0, 3).map((item, idx) => {
            ...
            return (
              ...
              >
                <Image
                  src={(item.type === 'video' ? (item.poster || '/images/protein.webp') : (item.src || '/images/protein.webp')).trim()}
                  alt={`Thumbnail ${idx + 1}`}
                  fill
                  className="object-cover"
