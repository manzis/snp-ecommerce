        ...
        <>
            ...
            <nav className={`fixed top-[12px] left-1/2 z-[100] w-full -translate-x-1/2 px-[13px] md:top-[24px] lg:top-0 lg:left-0 lg:translate-x-0 lg:w-full lg:max-w-none lg:px-0 transform-gpu ${isMounted ? 'transition-[transform] duration-300 ease-out' : ''} will-change-transform`}>
                ...
                >
                    ...
                    <div className="flex h-[60px] w-full items-center justify-between px-[14px] md:h-[72px] md:px-[24px] lg:h-[76px] lg:max-w-[1200px] lg:mx-auto lg:px-[24px]">
                        ...
                        >
                            <div className="h-[20px] w-[20px] md:h-[24px] md:w-[24px] lg:h-[20px] lg:w-[20px]">
                                <MenuIcon className={`h-full w-full ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${isScrolled ? 'text-[#242424]' : 'text-[#ffffff]'}`} />
                            </div>
                        </button>
                        ...
                        <div className="flex items-center gap-[24px] md:gap-[32px]">
                            ...
                            <Link
                                href="/track-order"
                                className={`flex h-[28px] items-center justify-center rounded-[6px] p-[2px_6px] ${isMounted ? 'transition-[background-color,transform] duration-300 ease-in-out' : ''} active:scale-95 md:h-[36px] md:px-[20px] lg:h-[32px] lg:px-[16px] ${
                                    isScrolled 
                                        ? 'bg-[#000000] hover:bg-[#1a1a1a]' 
                                        : 'bg-white hover:bg-[#f8fafc]'
                                }`}
                            >
                                <span className={`font-titillium text-[12px] font-semibold tracking-[0.2px] uppercase md:text-[13px] leading-[18px] ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${
                                    isScrolled 
                                        ? 'text-white' 
                                        : 'text-[#242424]'
                                }`}>
                            ...
                            >
                                <div className="h-[24px] w-[24px] md:h-[28px] md:w-[28px] lg:h-[24px] lg:w-[24px] mb-[2px]">
                                    <SearchIcon className={`h-full w-full ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${isScrolled ? 'text-[#242424]' : 'text-[#ffffff]'}`} />
                                </div>
                            </Link>
                            ...
                            >
                                <div className="relative h-[22px] w-[22px] md:h-[28px] md:w-[28px] lg:h-[22px] lg:w-[22px]">
                                    <CartIcon className={`h-full w-full ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${isScrolled ? 'text-[#242424]' : 'text-[#ffffff]'}`} />
                                    {cartCount > 0 && (
                                        <div className={`absolute -right-[12px] -top-[12px] flex h-[18px] min-w-[18px] items-center justify-center rounded-[6px] border-[1.5px] px-[5px] py-[2px] md:-right-[14px] md:-top-[14px] md:h-[22px] md:min-w-[22px] ${isMounted ? 'transition-[background-color,border-color] duration-300 ease-in-out' : ''} will-change-[background-color,border-color] ${
                                            isScrolled 
                                                ? 'border-white bg-[#242424]' 
                                                : 'border-[#242424] bg-white'
                                        }`}>
                                            <span className={`font-titillium text-[10px] font-normal leading-none md:text-[13px] ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${
                                                isScrolled 
                                                    ? 'text-white' 
                                                    : 'text-[#242424]'
                                            }`}>
                    ...
                    {showBanner && (
                        <div className={`flex w-full items-center justify-center py-[8px] md:py-[10px] ${isMounted ? 'transition-[background-color,border-radius] duration-300 ease-in-out' : ''} will-change-[background-color] ${
                            isScrolled 
                                ? 'rounded-[10px] bg-[#d6ff9c] lg:rounded-none lg:w-full lg:max-w-none' 
                                : 'bg-transparent'
                        }`}>
                            <span className={`font-titillium text-[14px] font-[400] leading-[18px] md:text-[15px] md:font-medium ${isMounted ? 'transition-colors duration-300' : ''} will-change-[color] ${
                                isScrolled 
                                    ? 'text-[#252525]' 
                                    : 'text-white'
                            }`}>
