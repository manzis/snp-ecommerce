...
const BrandCard: React.FC<BrandCardProps> = ({ brand, layout }) => {
  ...
  return (
    <Link 
      ...
      className={`group flex flex-col gap-[11px] shrink-0 transition-all active:scale-[0.98] cursor-pointer ${
        isPopular ? 'w-[147px]' : 'w-full'
      }`}
    >
      ...
      <div className="flex flex-col gap-[6px] items-start self-stretch">
        <h3 className="font-titillium text-[13px] font-semibold uppercase leading-[18px] tracking-[0.56px] text-[#242424] truncate w-full group-hover:text-[#308026] transition-colors">
          {brand.name}
        </h3>
