...
interface PaymentOptionProps {
  ...
  isPopular?: boolean;
  isSpecialRing?: boolean; // Used for the double-border effect (COD)
  isActive: boolean;
...
const PaymentOption: React.FC<PaymentOptionProps> = ({
  ...
  const inactiveGradient = 'linear-gradient(45deg, #FCFFFA 50%, #fafff3 100%)';
  ...
  return (
    ...
    >
      {/* 
          OUTER RING LOGIC: 
          Instead of using physical padding which causes "wrapping" jitters, 
          we use an absolute inset shadow or a transition-based ring.
      */}
      <div 
        className={`relative w-full transition-all duration-500 rounded-[18px] ${
          isSpecialRing && isActive 
            ? 'p-[2.5px] border-[1.5px] border-[#242424]' 
            : 'p-0 border-[1.5px] border-transparent'
        }`}
      >
        <motion.button
          layout="position"
          ...
          className={`flex w-full min-h-[56px] px-[16px] py-[16px] justify-between items-center transition-all duration-300 outline-none border-[1.5px] rounded-[16px] ${
            isActive ? 'border-[#242424]' : 'border-[#e2e8f0]'
          }`}
        >
